...
//     <>
//       {/* Navbar */}
//       <nav className="h-16 bg-[#B88455] flex justify-between items-center px-4 text-white fixed w-full z-50 shadow-md">
//         {/* Logo */}
//         <div>
...
//             isSidebarOpen ? "flex-col items-center fixed inset-0 bg-[#B88455] justify-center z-50" : "hidden md:flex"
//           }`}
//         >
//           <NavLink
//             to="/"
...
const Navbar = () => {
  const [isSidebarOpen, setIsSidebarOpen] = useState(false);
  const location = useLocation(); // To detect the current active route
        ...
        >
          {navItems.map((item) => (
            <NavLink
              ...
              to={item.path}
              onClick={closeSidebar}
              className="relative px-2 hover:text-yellow-300 transition-colors duration-300"
            >
              {item.name}
              {/* Underline Animation */}
              <div
                className={`absolute left-0 bottom-[-4px] h-1 w-full bg-yellow-300 transition-transform duration-300 ${
